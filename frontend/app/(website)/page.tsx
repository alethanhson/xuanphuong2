...
async function fetchData() {
  try {
    ...
    // Lấy thiết lập hero section từ API
    const heroSettingsResponse = await WebsiteSettingsService.getHeroSectionSettings();

    return {
      products: productsResponse.data?.products || [],
      heroSlides: heroSettingsResponse.data?.slides || FALLBACK_HERO_SLIDES
    };
  } catch (error) {
    console.error("Lỗi khi lấy dữ liệu cho trang chủ:", error);
...
export default async function HomePage() {
  // Fetch data from API
  const { products, heroSlides } = await fetchData();
